...
var DirectCryptTokenPreICO = artifacts.require("./DirectCryptTokenPreICO.sol");
...
module.exports = async function(deployer) {
  ...
  deployer.link(SafeMath, DirectCryptTokenPreICO);
  await deployer.deploy(DirectCryptToken).then(async function() {
    ...
    await deployer.deploy(DirectCryptTokenPreICO, 
      hardCap,
      softCap,
      token,
      beneficiary,
    ...
    const preSaleInstance = await web3.eth.contract(DirectCryptTokenPreICO.abi).at(DirectCryptTokenPreICO.address);
    ...
    await ethProvider.setWatcher(DirectCryptTokenPreICO.address, { from: web3.eth.accounts[0] });
    await btcProvider.setWatcher(DirectCryptTokenPreICO.address, { from: web3.eth.accounts[0] });
